...
| Tests | 312 passing under the supported verify path after editable install or `PYTHONPATH=src`; raw-checkout `pytest -q` is not a supported path |
| CI | Fresh-env test gate + offline fixture smoke plus golden snapshot gate + offline security hygiene gate + exa-py compatibility matrix + release-scorecard artifact job with artifact validation, all using editable package install and categorized quality-gate artifacts |
...
- CI gates now emit categorized quality-gate JSON, Markdown, and log artifacts via `python -m war_room.quality_gates`, separating unit, offline fixture, golden snapshot, Exa compatibility, release-scorecard, and security-hygiene failures.
...
- #9 expanded CI quality gates (failure-categorization artifacts and offline security hygiene gate landed; broader integration/e2e gates still open)
