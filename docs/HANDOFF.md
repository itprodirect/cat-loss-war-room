...
| Experimental Codex security scan | Enabled on `main` as an advisory signal for follow-up triage only; it is not a formal audit, production security baseline, compliance control, or substitute for the existing offline hygiene gates |
...
| Evidence/provenance adapters and dedupe (`#12`) | First narrow adapter seams have landed for current weather, carrier, caselaw, and citation-verification module output: issue `#94` / PR `#95` added `caselaw_pack_to_evidence_items(...)`, issue `#96` / PR `#97` added `carrier_doc_pack_to_evidence_items(...)`, issue `#98` / PR `#99` added `weather_brief_to_evidence_items(...)`, and issue `#103` / PR `#105` added `citation_verify_pack_to_evidence_items(...)`; issue `#107` / PR `#108` added deterministic `dedupe_evidence_items(...)`; PR `#136` integrated same-module audit-snapshot dedupe with `old_id -> retained_id` remapping; PR `#138` added citation `not_found` regression coverage and markdown raw-vs-retained count visibility; issue `#139` keeps cross-module behavior clustering-only. These slices are not a full V2 evidence graph, database, dashboard, API, review workflow, or product runtime |
...
- The issue `#107` / PR `#108` deterministic evidence dedupe helper provides `dedupe_evidence_items(...)` over canonical `EvidenceItem` rows. PR `#136` then wired same-module dedupe into audit snapshot assembly with an explicit `old_id -> retained_id` remapping, and PR `#138` surfaced raw/pre-dedupe versus retained/exported evidence counts in markdown.
- Issue `#139` records the current cross-module dedupe decision: caselaw and citation-verification rows can share an `EvidenceCluster`, but cross-module rows do not collapse.
...
- [ISSUE_139_RETAINED_DUPLICATE_SOURCE_ROLE_DECISION.md](ISSUE_139_RETAINED_DUPLICATE_SOURCE_ROLE_DECISION.md): decision record keeping cross-module relatedness clustering-only and placing any future same-module duplicate/source-role metadata in a separate audit trace object
