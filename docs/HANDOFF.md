...
| Tests | 249 passing under editable install or `PYTHONPATH=src`; raw-checkout `pytest -q` is not a supported path |
...
- The notebook and preflight surfaces now expose a workflow-oriented research-plan preview, evidence-board summary, issue-workspace summary, memo-composer summary, and run timeline, so grouped support, issue-level review, section readiness, and review-required state are visible before the memo is treated as complete.
