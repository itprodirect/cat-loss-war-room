...
- Current branch: `codex/issue-6-export-history-contract`
- Last validated: 2026-04-29 via `$env:PYTHONPATH='src'; python -m war_room --verify --release-candidate issue-6-export-history-contract` (`291 passed`; offline preflight passed for 4 fixture scenarios)
- Current focus: `#6` Export History read-model contract slice: export artifact rows now use a typed `v2alpha1` payload adapter before notebook-friendly rendering.
- Hot files: `README.md`, `CLAUDE.md`, `docs/BUILD_CHECKLIST.md`, `docs/HANDOFF.md`, `docs/ROADMAP.md`, `docs/V2_ISSUE_MAP.md`, `docs/V2_RELEASE_RUBRIC.md`, `docs/heartbeat.md`, `docs/SESSION_LOG.md`, `logs/2026-04-28-session.md`, `src/war_room/models.py`, `src/war_room/export_history.py`, `src/war_room/__init__.py`, `tests/test_export_history.py`
...
- Next best task: Review and merge the `#6` Export History read-model PR, then reassess whether `#6` is ready to close or whether the Run Timeline read model needs one final contract pass.
