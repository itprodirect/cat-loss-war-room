...
- Current branch: `codex/issue-6-memo-composer-contract`
- Last validated: 2026-04-28 local / 2026-04-29 UTC via `$env:PYTHONPATH='src'; python -m war_room --verify --release-candidate issue-6-memo-composer-contract` (`289 passed`; offline preflight passed for 4 fixture scenarios)
- Current focus: `#6` Memo Composer read-model contract slice: section cards now use a typed `v2alpha1` payload adapter before notebook-friendly rendering.
- Hot files: `README.md`, `CLAUDE.md`, `docs/BUILD_CHECKLIST.md`, `docs/HANDOFF.md`, `docs/ROADMAP.md`, `docs/V2_ISSUE_MAP.md`, `docs/V2_RELEASE_RUBRIC.md`, `docs/heartbeat.md`, `docs/SESSION_LOG.md`, `logs/2026-04-28-session.md`, `src/war_room/models.py`, `src/war_room/memo_composer.py`, `src/war_room/__init__.py`, `tests/test_memo_composer.py`
...
- Next best task: Review and merge the `#6` Memo Composer read-model PR, then continue with the next small read-model contract seam only where it has clear workflow/runtime payoff.
