...
- 449 tests are passing on the supported verify path.
...
- The issue `#92` / PR `#93` release-evidence CI reporting summary maps the existing verify bundle, scorecard artifacts, `reviewer_summary`, and blocking/advisory readiness fields for reporting consumers without adding a dashboard or second readiness model.
...
- First narrow issue `#12` evidence/provenance adapter slices have landed for current caselaw, carrier, and weather module output. They route those module outputs through canonical `EvidenceItem` adapters with deterministic provenance-oriented IDs while preserving the current notebook-era audit snapshot flow.
...
- Issue [#12](https://github.com/itprodirect/cat-loss-war-room/issues/12) has first adapter slices landed for caselaw issue `#94`, carrier issue `#96`, and weather issue `#98`. Citation-verify adaptation, broader dedupe/provenance hardening, persistence, dashboards, API integration, and any claim that the full V2 evidence graph is complete remain pending.
...
- V2 implementation work now has first narrow issue `#12` evidence-adapter slices for caselaw, carrier, and weather output; broader provenance/dedupe, product workflow, API, UI, persistence, review, and pilot operationalization work remains pending.
...
1. [#12](https://github.com/itprodirect/cat-loss-war-room/issues/12) Continue evidence/provenance adapter work after the landed caselaw, carrier, and weather slices
...
- The main cleanup is scope clarity, not deletion:
  ...
  - `#12` should explicitly implement against the canonical schema defined in `#24`; caselaw, carrier, and weather adapter slices have landed, but the full evidence/provenance lane is still open.
...
- [#12](https://github.com/itprodirect/cat-loss-war-room/issues/12) Review remaining evidence/provenance adapter scope, then either add a citation-verify adapter as the next narrow slice or pause for an adapter-status map.
...
- [#12](https://github.com/itprodirect/cat-loss-war-room/issues/12) Evidence normalization + provenance beyond the landed caselaw, carrier, and weather adapters
