...
## 8) Current Baseline Snapshot (May 20, 2026)
...
|---|---:|---|---|
| Reliability | 3 | Strong | `455` tests pass on the supported verify path, CI covers fresh-env plus `exa-py` compatibility plus offline fixture smoke/golden snapshot validation, offline e2e demo validation, offline security and dependency hygiene, and release-scorecard artifact validation, and the committed five-scenario FL/TX/LA lane still meets the calibrated demo-ready thresholds. |
| Evidence Quality | 2 | Acceptable | The committed five-scenario fixture set still satisfies explicit demo-ready thresholds for scenario count, state coverage, issue breadth, citation coverage, module completeness, source mix, output structure, and citation-summary consistency, with all five fixture lanes now represented as offline-ready curated registry scenarios. First issue `#12` weather, carrier, caselaw, and citation-verification adapters now route current module output into canonical `EvidenceItem` rows with deterministic provenance-oriented IDs. Broader scenario breadth, richer dedupe/provenance normalization, citation-quality regression, and product review workflow still remain open under `#12`, `#13`, and `#14`; any additional Florida fixture seeding should be scoped by maintainers as follow-up work after the completed `#8` baseline. |
| Trust and Provenance | 2 | Acceptable | Disclaimers, source tiers, citation checks, evidence clusters, claim/review trace links, and first module-to-`EvidenceItem` adapter seams exist, but they are still notebook-era rather than full product workflow state. |
...
python -m war_room.release_scorecard \
  ...
  --verification-summary "455 passed"
```
...
- `#12`, `#13`, and `#14`: broader evidence normalization, dedupe/provenance hardening, citation-quality regression, and case-law quality
