...
| Scenario fixture suite | [#8](https://github.com/itprodirect/cat-loss-war-room/issues/8) | Four committed scenario directories now cover Florida, Texas, and Louisiana and feed `#27`; Milton and Ida now map from the curated registry to offline-ready fixture lanes; a first deterministic golden snapshot gate covers output structure, source mix, case counts, citation summaries, and coverage metadata; `docs/FIXTURE_SEEDING.md` defines the safe promotion process and scenario tests guard offline-ready fixture links; the next step is broader fixture breadth |
