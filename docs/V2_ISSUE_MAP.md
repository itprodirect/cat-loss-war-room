...
| Scenario fixture suite | [#8](https://github.com/itprodirect/cat-loss-war-room/issues/8) | Five committed scenario directories now cover Florida, Texas, and Louisiana and feed `#27`; all five now map from the curated registry to offline-ready fixture lanes, including Ian/Lee/Citizens HO-3 and both Texas hail/Tarrant/Allstate HO-B and DP-3 benchmarks; the golden snapshot gate covers output structure, source mix, case counts, citation summaries, and coverage metadata; `docs/FIXTURE_SEEDING.md` defines the safe promotion process, `docs/ISSUE_8_NEXT_SCENARIO_AUDIT.md` tracks the remaining live-only seeding candidates, and `docs/ISSUE_8_READINESS_AUDIT.md` keeps `#8` open for maintainer review after the fifth fixture lane |
