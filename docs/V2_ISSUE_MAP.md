...
| CI quality gate pipeline | [#9](https://github.com/itprodirect/cat-loss-war-room/issues/9) | Fresh-env + exa compatibility + offline fixture smoke now exist, with the fixture smoke lane also checking the committed golden snapshot; release-scorecard artifact emission and validation plus the local verify-evidence workflow are wired; quality-gate artifacts now separate unit, offline fixture, offline e2e, golden snapshot, Exa compatibility, release-scorecard, security-hygiene, and dependency-hygiene failures, and the issue is ready for closeout review after the dependency-hygiene slice lands |
