...
| Focus | Issue | Notes |
|---|---|---|
| Typed domain models | [#6](https://github.com/itprodirect/cat-loss-war-room/issues/6) | Complete with closeout audit: intake/query, pack, citation/export, graph/version, issue/authority, run/retrieval, review/export graph-linkage, schema-versioned runtime cache-envelope, and typed Run Timeline / Evidence Board / Issue Workspace / Memo Composer / Export History read-model slices are landed against the canonical schema in `#24`; closeout audit lives in `docs/ISSUE_6_CLOSEOUT_AUDIT.md` |
| Retrieval adapter contract tests | [#7](https://github.com/itprodirect/cat-loss-war-room/issues/7) | Complete: provider seam, notebook retrieval-state emission, citation-verify tracking, deterministic retrieval-task timing, Exa compatibility tests, provider failure-mode normalization, and `None` provider-response malformed-contract handling are landed; `docs/ISSUE_7_CLOSURE_SANITY_AUDIT.md` records the PR #57 follow-up |
| Scenario fixture suite | [#8](https://github.com/itprodirect/cat-loss-war-room/issues/8) | Four committed scenario directories now cover Florida, Texas, and Louisiana and feed `#27`; all four now map from the curated registry to offline-ready fixture lanes, including both Texas hail/Tarrant/Allstate HO-B and DP-3 benchmarks; the golden snapshot gate covers output structure, source mix, case counts, citation summaries, and coverage metadata; `docs/FIXTURE_SEEDING.md` defines the safe promotion process, `docs/ISSUE_8_NEXT_SCENARIO_AUDIT.md` identifies the remaining live-only seeding candidates, and `docs/ISSUE_8_READINESS_AUDIT.md` confirms `#8` should remain open until a fifth committed fixture lane is seeded or that scope moves to a follow-up issue |
| CI quality gate pipeline | [#9](https://github.com/itprodirect/cat-loss-war-room/issues/9) | Complete with closeout audit in `docs/ISSUE_9_CLOSEOUT_AUDIT.md`: fresh-env tests, Exa compatibility, offline fixture smoke, golden snapshot tests and direct snapshot diff, offline e2e, security hygiene, dependency hygiene, release-scorecard generation and validation, categorized quality-gate artifacts, and offline demo/fixture compatibility are all wired |
