...
| Scenario fixture suite | [#8](https://github.com/itprodirect/cat-loss-war-room/issues/8) | Four committed scenario directories now cover Florida, Texas, and Louisiana and feed `#27`; Milton, Ida, and Texas hail/Tarrant/Allstate HO-B now map from the curated registry to offline-ready fixture lanes; the golden snapshot gate covers output structure, source mix, case counts, citation summaries, and coverage metadata; `docs/FIXTURE_SEEDING.md` defines the safe promotion process and `docs/ISSUE_8_NEXT_SCENARIO_AUDIT.md` identifies the next candidates; the next step is broader fixture breadth |
